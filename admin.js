...
function iniciarPanelAdmin() {
    ...
    precargarCatalogoProductosAutomatico();
    setInterval(() => { sincronizarDatosGrid(true); }, 30000);
}

function precargarCatalogoProductosAutomatico() {
    ...
    .catch(err => console.error("Error al precargar catálogo:", err));
}
...
function toggleEstadoTiendaInternet() {
    const tokenActual = localStorage.getItem("ADMIN_TACOS_TOKEN");
    if (!tokenActual) {
...
// ==========================================================================
// 3. GESTIÓN COMPLETA DEL MENÚ E INVENTARIO (CREAR, EDITAR Y ELIMINAR)
// ==========================================================================
    ...
    arrProductos.forEach(p => {
        ...
        row.innerHTML = `
            <div class="menu-mgmt-info">
                ...
                <span class="menu-mgmt-category">${p.categoria || 'General'} ${p.propietario ? '• 👤 ' + p.propietario : ''}</span>
            </div>
            
            <div class="menu-mgmt-controls">
                <span style="font-size:0.8rem; color:#ff944d; font-weight:800;">$${precioNum.toFixed(2)}</span>

                <button class="filter-btn-custom" onclick="editarProductoModal('${prodId}')" style="background:#222; color:#ff5500; border:1px solid rgba(255,85,0,0.3); padding:6px 10px; font-size:0.78rem;" title="Editar Todo">
                    <i class="fa-solid fa-pen-to-square"></i>
                </button>

                <button class="filter-btn-custom" onclick="eliminarProductoExpress('${prodId}')" style="background:rgba(255,51,68,0.1); color:#ff3344; border:1px solid rgba(255,51,68,0.3); padding:6px 10px; font-size:0.78rem;" title="Eliminar Producto">
...
function prepararFormularioNuevoProducto() {
    ...
    document.getElementById("formProductTitle").innerText = "Crear Nuevo Producto";
    document.getElementById("prodNombre").value = "";
    document.getElementById("prodCategoria").value = "";
    document.getElementById("prodDescripcion").value = "";
    document.getElementById("prodPrecio").value = "";
    document.getElementById("prodAgotado").value = "NO";
    document.getElementById("prodPropietario").value = "Gerardo";
    document.getElementById("prodImagen").value = "";

    document.getElementById("productFormBox").style.display = "block";
}

function editarProductoModal(productId) {
    const prod = listaProductosMenu.find(p => p.id.toString().trim() === productId);
    if (!prod) return;

    document.getElementById("prodEditId").value = prod.id;
    document.getElementById("formProductTitle").innerText = `Editar: ${prod.nombre}`;
    ...
    document.getElementById("prodAgotado").value = (prod.agotado || "NO").toUpperCase();
    document.getElementById("prodPropietario").value = prod.propietario || "Gerardo";
...
function guardarProductoSubmit(event) {
    event.preventDefault();
    const tokenActual = localStorage.getItem("ADMIN_TACOS_TOKEN");
    if (!tokenActual) {
        alert("⚠️ Sesión expirada.");
    ...
    const idEdit = document.getElementById("prodEditId").value.trim();
    const nombre = document.getElementById("prodNombre").value.trim();
    const categoria = document.getElementById("prodCategoria").value.trim();
    const descripcion = document.getElementById("prodDescripcion").value.trim();
    const precio = document.getElementById("prodPrecio").value;
    const agotado = document.getElementById("prodAgotado").value;
    const propietario = document.getElementById("prodPropietario").value.trim() || "Gerardo";
    const imagen = document.getElementById("prodImagen").value.trim();

    const tipoAccion = idEdit ? "EDITAR_PRODUCTO" : "CREAR_PRODUCTO";
...
function toggleAgotadoExpress(productId, estaAgotadoChecked) {
    ...
    .then(data => {
        if (data.result === "success" || data.status === "SUCCESS") {
            ...
            alert("❌ Error de actualización: " + (data.message || "Acceso denegado o token inválido."));
            const switchEl = document.getElementById(`switch-${productId}`);
            if (switchEl) switchEl.checked = !estaAgotadoChecked;
            if (labelState) {
                labelState.innerText = !estaAgotadoChecked ? "AGOTADO" : "DISPONIBLE";
                labelState.style.color = !estaAgotadoChecked ? "#ff3344" : "#25D366";
            }
...
function eliminarProductoExpress(productId) {
    const tokenActual = localStorage.getItem("ADMIN_TACOS_TOKEN");
    if (!tokenActual) {
        alert("⚠️ Sesión expirada.");
        cerrarSesionAdmin();
    ...
    const prod = listaProductosMenu.find(p => p.id.toString().trim() === productId);
    const nombreProd = prod ? prod.nombre : productId;

    if (!confirm(`⚠️ ¿Estás seguro de que deseas eliminar permanentemente el producto "${nombreProd}"? Esta acción borrará la fila en tu base de datos.`)) {
        return;
    }
...
// ==========================================================================
// 4. MÓDULO DE GESTIÓN DE COMBOS Y PROMOCIONES
// ==========================================================================
function abrirModalGestionPromos() {
    const modal = document.getElementById("promosMgmtModal");
    const container = document.getElementById("promosMgmtList");
    cancelarFormularioPromo();
    
    modal.classList.add("show");
    container.innerHTML = `<p style="text-align:center; padding:30px; color:#00d4ff;"><i class="fa-solid fa-spinner fa-spin"></i> Cargando promociones...</p>`;

    fetch(GOOGLE_SCRIPT_URL, { method: "GET", redirect: "follow" })
    .then(res => res.json())
    .then(data => {
        listaPromocionesAdmin = data.promociones || [];
        renderizarListaGestionPromos(listaPromocionesAdmin);
    })
    .catch(() => { container.innerHTML = `<p style="text-align:center; color:#ff3344;">❌ Error al conectar con la base de promociones.</p>`; });
}

function cerrarModalGestionPromos() { document.getElementById("promosMgmtModal").classList.remove("show"); }

function renderizarListaGestionPromos(arrPromos) {
    const container = document.getElementById("promosMgmtList");
    container.innerHTML = "";

    if (!arrPromos || arrPromos.length === 0) {
        container.innerHTML = `<p style="text-align:center; color:#888; padding:20px;">No hay promociones registradas.</p>`;
        return;
    }

    arrPromos.forEach(p => {
        const fila = p.filaExcel;
        const estaActivo = (p.activo || "").toUpperCase() === "SI";
        const pLista = parseFloat(p.precioLista || 0).toFixed(2);
        const pOferta = parseFloat(p.precioOferta || 0).toFixed(2);
        ...
        card.innerHTML = `
            <div class="promo-card-header">
                <div>
                    <span class="promo-days-tag">${p.diaSemana || 'Cualquier día'}</span>
                    <h4 style="color:#fff; font-size:1rem; margin-top:4px;">${p.titulo}</h4>
                </div>
                <div class="promo-prices">
                    <del>$${pLista}</del> $${pOferta}
                </div>
            </div>
            
            <p style="font-size:0.82rem; color:#aaa; margin-bottom:10px;">${p.descripcion || ''}</p>
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; border-top:1px dashed rgba(255,255,255,0.05); padding-top:10px;">
                <div style="display:flex; gap:8px;">
                    <button class="filter-btn-custom" onclick="editarPromoExpress(${fila})" style="background:#222; color:#00d4ff; border:1px solid rgba(0,212,255,0.3); padding:6px 12px; font-size:0.78rem;">
                        <i class="fa-solid fa-pen-to-square"></i> Editar
                    </button>
                    <button class="filter-btn-custom" onclick="eliminarPromoExpress(${fila})" style="background:rgba(255,51,68,0.1); color:#ff3344; border:1px solid rgba(255,51,68,0.3); padding:6px 12px; font-size:0.78rem;">
                        <i class="fa-solid fa-trash-can"></i>
                    </button>
                </div>

                <div style="display:flex; align-items:center; gap:8px;">
                    <span style="font-size:0.75rem; color:${estaActivo ? '#25D366' : '#ff3344'}; font-weight:800;">
                        ${estaActivo ? 'ACTIVO' : 'PAUSADO'}
                    </span>
                    <label class="switch-toggle">
                        <input type="checkbox" ${estaActivo ? 'checked' : ''} onchange="toggleEstadoPromoExpress(${fila}, this.checked)">
                        <span class="slider-round"></span>
                    </label>
                </div>
            </div>
...
function prepararFormularioNuevaPromo() {
    document.getElementById("promoFilaExcel").value = "";
    document.getElementById("formPromoTitle").innerText = "Crear Nuevo Combo / Promoción";
    document.getElementById("promoTitulo").value = "";
    document.getElementById("promoSubtitulo").value = "";
    document.getElementById("promoDescripcion").value = "";
    document.getElementById("promoPrecioLista").value = "";
    document.getElementById("promoPrecioOferta").value = "";
    document.getElementById("promoConfigOpciones").value = "";

    document.querySelectorAll("input[name='chkDia']").forEach(chk => chk.checked = false);

    document.getElementById("promoFormBox").style.display = "block";
}

function editarPromoExpress(filaExcel) {
    const promo = listaPromocionesAdmin.find(p => p.filaExcel === filaExcel);
    if (!promo) return;

    document.getElementById("promoFilaExcel").value = filaExcel;
    document.getElementById("formPromoTitle").innerText = `Editar: ${promo.titulo}`;
    document.getElementById("promoTitulo").value = promo.titulo || "";
    document.getElementById("promoSubtitulo").value = promo.subtitulo || "";
    document.getElementById("promoDescripcion").value = promo.descripcion || "";
    document.getElementById("promoPrecioLista").value = promo.precioLista || "";
    document.getElementById("promoPrecioOferta").value = promo.precioOferta || "";
    document.getElementById("promoConfigOpciones").value = promo.configOpciones || "";

    const diasStr = (promo.diaSemana || "").toLowerCase();
    document.querySelectorAll("input[name='chkDia']").forEach(chk => {
        chk.checked = diasStr.includes(chk.value.toLowerCase());
    });

    document.getElementById("promoFormBox").style.display = "block";
}

function cancelarFormularioPromo() {
    document.getElementById("promoFormBox").style.display = "none";
}

function guardarPromoSubmit(event) {
    event.preventDefault();
    const tokenActual = localStorage.getItem("ADMIN_TACOS_TOKEN");
    if (!tokenActual) {
        alert("⚠️ Sesión expirada.");
    ...
    const filaExcel = document.getElementById("promoFilaExcel").value;
    const titulo = document.getElementById("promoTitulo").value.trim();
    const subtitulo = document.getElementById("promoSubtitulo").value.trim();
    const descripcion = document.getElementById("promoDescripcion").value.trim();
    const precioLista = document.getElementById("promoPrecioLista").value;
    const precioOferta = document.getElementById("promoPrecioOferta").value;
    const configOpciones = document.getElementById("promoConfigOpciones").value.trim();

    let diasSeleccionados = [];
    document.querySelectorAll("input[name='chkDia']:checked").forEach(chk => {
    ...
    fetch(GOOGLE_SCRIPT_URL, {
        ...
        body: JSON.stringify({
            action: "guardarPromo",
            filaExcel: filaExcel ? parseInt(filaExcel) : null,
    ...
    .then(data => {
        btn.disabled = false;
        btn.innerHTML = `<i class="fa-solid fa-floppy-disk"></i> Guardar Combo`;

        if (data.result === "success") {
            cancelarFormularioPromo();
            abrirModalGestionPromos();
        } else {
            alert("❌ Error: " + (data.message || "No se pudo guardar la oferta."));
        }
    })
    .catch(() => {
        btn.disabled = false;
...
function toggleEstadoPromoExpress(filaExcel, estaActivoChecked) {
    const tokenActual = localStorage.getItem("ADMIN_TACOS_TOKEN");
    if (!tokenActual) {
        alert("⚠️ Sesión expirada.");
        cerrarSesionAdmin();
    ...
    const nuevoEstado = estaActivoChecked ? "SI" : "NO";

    fetch(GOOGLE_SCRIPT_URL, {
        method: "POST",
        headers: { "Content-Type": "text/plain;charset=utf-8" },
        body: JSON.stringify({
            action: "toggleEstadoPromo",
            filaExcel: filaExcel,
    ...
    .then(data => {
        if (data.result === "success") {
            const promo = listaPromocionesAdmin.find(p => p.filaExcel === filaExcel);
            if (promo) promo.activo = nuevoEstado;
            renderizarListaGestionPromos(listaPromocionesAdmin);
        } else {
...
function eliminarPromoExpress(filaExcel) {
    const tokenActual = localStorage.getItem("ADMIN_TACOS_TOKEN");
    if (!tokenActual) {
        alert("⚠️ Sesión expirada.");
        cerrarSesionAdmin();
    ...
    if (!confirm("¿Seguro que deseas eliminar este combo permanentemente?")) return;

    fetch(GOOGLE_SCRIPT_URL, {
        method: "POST",
        headers: { "Content-Type": "text/plain;charset=utf-8" },
        body: JSON.stringify({
            action: "eliminarPromo",
            filaExcel: filaExcel,
            token: tokenActual
        })
    })
...
function generarCierreDelDiaPDF() {
    ...
    if (!window.listaProductosMenu || window.listaProductosMenu.length === 0) {
        fetch(GOOGLE_SCRIPT_URL, { method: "GET", redirect: "follow" })
        .then(res => res.json())
        .then(data => {
            if (data && Array.isArray(data.productos)) {
                window.listaProductosMenu = data.productos;
                generarCierreDelDiaPDF();
            } else {
                alert("⚠️ No se pudo cargar el catálogo de dueños. Abre primero el menú de inventario un segundo para sincronizar.");
            }
        })
        .catch(() => alert("❌ Error de conexión al sincronizar el inventario para el reporte."));
        return;
    }
    ...
    let ventasPorPropietarioDetalle = {};
    let platillosCanceladosLista = [];
    let menuMap = {};
    let listaPropietariosUnicos = new Set();

    window.listaProductosMenu.forEach(prod => {
        if (prod.nombre) {
            let owner = (prod.propietario || "Gerardo").trim();
            listaPropietariosUnicos.add(owner);
            menuMap[prod.nombre.trim().toLowerCase()] = {
                propietario: owner,
                precio: parseFloat(prod.precio) || 0
            };
        }
    });
    listaPropietariosUnicos.add("Gerardo");

    filasFiltradasGlobal.forEach(p => {
        let totalNum = parseFloat(p.total) || 0;
        let estatus = (p.estatus || "PENDIENTE").toUpperCase();

        if (estatus === "CANCELADO") {
            cantCancelados++;
            if (p.productos) {
                p.productos.split("|").forEach(itemRaw => {
                    let itemStr = itemRaw.trim();
                    if (itemStr) platillosCanceladosLista.push(`${itemStr} (Pedido de: ${p.cliente || 'General'} - $${totalNum.toFixed(2)})`);
                });
            }
            ...
            if (p.productos) {
                let itemsList = p.productos.split("|");
                itemsList.forEach(itemRaw => {
                    let itemStr = itemRaw.trim();
                    if (!itemStr) return;
                    let match = itemStr.match(/^(\d+)x\s+(.*)$/);
                    let qty = 1;
                    let prodName = itemStr;
                    if (match) {
                        qty = parseInt(match[1]) || 1;
                        prodName = match[2].trim();
                    }
                    let keyName = prodName.toLowerCase();
                    let owner = "Gerardo";
                    let unitPrice = 0;

                    if (menuMap[keyName]) {
                        owner = menuMap[keyName].propietario;
                        unitPrice = menuMap[keyName].precio;
                    } else {
                        let foundKey = Object.keys(menuMap).find(k => keyName.includes(k) || k.includes(keyName));
                        if (foundKey) {
                            owner = menuMap[foundKey].propietario;
                            unitPrice = menuMap[foundKey].precio;
                        }
                    }

                    if (!ventasPorPropietarioDetalle[owner]) {
                        ventasPorPropietarioDetalle[owner] = { montoTotal: 0, items: [] };
                    }
                    let subtotalItem = unitPrice > 0 ? (unitPrice * qty) : (totalNum / itemsList.length);
                    ventasPorPropietarioDetalle[owner].montoTotal += subtotalItem;
                    ventasPorPropietarioDetalle[owner].items.push(`${qty}x ${prodName} ($${subtotalItem.toFixed(2)})`);
                });
            }
        }
    });

    let propietariosHTML = "";
    listaPropietariosUnicos.forEach(owner => {
        let datosOwner = ventasPorPropietarioDetalle[owner] || { montoTotal: 0, items: [] };
        if (datosOwner.items.length === 0) return;

        let listaItemsHtml = datosOwner.items.map(item => `<li>${item}</li>`).join("");

        propietariosHTML += `
            <div style="margin-bottom: 20px; border: 1px solid #ccc; padding: 12px; border-radius: 6px; background: #fff;">
                <h3 style="margin: 0 0 5px 0; color: #222;">👤 Propietario: ${owner}</h3>
                <p style="margin: 0 0 10px 0; font-size: 15px; font-weight: bold; color: #2e7d32;">Venta Total del Dueño: $${datosOwner.montoTotal.toFixed(2)}</p>
                <div style="font-size: 13px; color: #444;">
                    <strong>Desglose de Platillos Entregados:</strong>
    ...
    let canceladosHTML = "";
    if (platillosCanceladosLista.length > 0) {
        let listaCanceladosHtml = platillosCanceladosLista.map(item => `<li>${item}</li>`).join("");
        canceladosHTML = `
            <div style="margin-top: 25px; border: 1px solid #ff3344; padding: 12px; border-radius: 6px; background: #fff5f5;">
                <h3 style="margin: 0 0 5px 0; color: #c62828;">❌ Platillos / Pedidos Cancelados</h3>
                <div style="font-size: 13px; color: #555;">
                    <ul style="color: #c62828;">${listaCanceladosHtml}</ul>
                </div>
            </div>
        `;
    }

    const ventanaPdf = window.open('', '_blank', 'width=650,height=750');
    ventanaPdf.document.write(`
        <html>
        <head>
            <title>Cierre de Caja por Propietario</title>
            <style>
                body { font-family: Arial, sans-serif; padding: 20px; color: #111; background: #fdfdfd; }
                h2, h3 { text-align: center; margin: 5px 0; }
                .resumen-box { background: #f1f1f1; border: 1px solid #ccc; padding: 12px; margin-bottom: 20px; border-radius: 6px; }
                ul { margin: 5px 0; padding-left: 20px; }
                .line { border-bottom: 2px solid #333; margin: 15px 0; }
            </style>
        </head>
        <body>
            <h2>TACOS & PAPAS ASADAS</h2>
            <h3>REPORTE DE CIERRE DE CAJA DESGLOSADO POR PROPIETARIO</h3>
            <p style="text-align:center; font-size: 12px; color: #666;">Fecha de impresión: ${new Date().toLocaleString()}</p>
            <div class="line"></div>
            
            <div class="resumen-box">
                <p><b>Pedidos Entregados:</b> ${cantEntregados}</p>
                <p><b>Pedidos Pendientes:</b> ${cantPendientes}</p>
                <p><b>Pedidos Cancelados:</b> ${cantCancelados}</p>
                <p style="font-size: 16px; color: #c62828;"><b>Venta Neta Real (Solo Entregados):</b> $${totalVentasNetas.toFixed(2)}</p>
            </div>

            <h3>DESGLOSE FINANCIERO POR DUEÑO</h3>
            ${propietariosHTML}
            ${canceladosHTML}
...
window.onclick = function(event) { 
    ...
    if (event.target === document.getElementById("promosMgmtModal")) cerrarModalGestionPromos(); 
}
...
// ==========================================================================
// CAMBIAR ESTATUS DIRECTO DE PEDIDO (ENTREGADO, CANCELADO, PENDIENTE)
// ==========================================================================
function cambiarEstatusDirecto(index, nuevoEstatus) {
    ...
    const pedido = filasFiltradasGlobal[index];
    if (!pedido) return alert("❌ No se encontró el pedido seleccionado.");

    const estatusAnterior = pedido.estatus;
    pedido.estatus = nuevoEstatus;
    filtrarTabla(); // Actualizar UI de forma inmediata

    fetch(GOOGLE_SCRIPT_URL, {
        method: "POST",
        headers: { "Content-Type": "text/plain;charset=utf-8" },
        body: JSON.stringify({
            action: "actualizarEstatusPedido",
    ...
    .then(data => {
        if (data.result !== "success" && data.status !== "SUCCESS") {
            ...
            pedido.estatus = estatusAnterior;
            filtrarTabla();
        }
    })
    .catch(err => {
        console.error("Error al actualizar estatus:", err);
        alert("❌ Error de red al intentar guardar el cambio.");
        pedido.estatus = estatusAnterior;
        filtrarTabla();
    });
}