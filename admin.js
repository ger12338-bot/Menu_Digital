...
function sincronizarDatosGrid(esSilencioso = false) {
    ...
    .then(data => {
        if (Array.isArray(data)) {
            ...
            // SONIDO TRIPLE AL DETECTAR UN PEDIDO NUEVO EN VIVO
...
function reproducirSonidoNuevoPedidoPotente() {
    try {
        ...
        function emitirBeep(tiempoInicio) {
            ...
            osc.type = "square"; 
            osc.frequency.setValueAtTime(880, tiempoInicio); 
            ...
            osc.stop(tiempoInicio + 0.18);
        }

        const ahora = audioCtx.currentTime;
        emitirBeep(ahora);
        emitirBeep(ahora + 0.25);
        emitirBeep(ahora + 0.50);
    } catch(e) { console.error("Audio bloqueado por navegador", e); }
}
...
// FILTRADO Y CLASIFICACIÓN DE CANAL (WHATSAPP vs MOSTRADOR)
function filtrarTabla() {
    ...
    filasFiltradasGlobal = datosFilas.filter(f => {
        // Lee la Columna I (método de pago) o referencias/gps
        let metodoPagoTexto = (f.metodo_pago || f['método de pago'] || '').toString().toLowerCase();
        
        // REGLA CLAVE: Si contiene "whatsapp", es canal WhatsApp. Cualquier otra cosa (Efectivo, Tarjeta, Transferencia) es Mostrador.
    ...
    filasFiltradasGlobal.forEach((f, index) => {
        ...
        // Renderiza únicamente las 2 etiquetas solicitadas
        let badgeHTML = esCanalWhatsApp 
...
function actualizarKPIS(pedidos) {
    let sumaTotal = pedidos.reduce((acc, f) => acc + (parseFloat(f.total) || 0), 0);
...
function imprimirTicketDirecto() {
    ...
    if (p.productos) {
        p.productos.split("|").forEach(prod => {
            if(prod.trim() !== "") itemsHTML += `<div style="display:flex; justify-content:space-between; margin:3px 0;"><span>${prod.trim()}</span></div>`;
        });
    }
    ...
    ventanaImp.document.write(`
        ...
        <body>
            ...
            <div class="line"></div>
            <div class="bold" style="margin-bottom:5px;">PLATILLOS:</div>
            ${itemsHTML}
            <div class="line"></div>
...
function generarCierreDelDiaCSV() {
    ...
    csvContent += "FECHA/HORA,CLIENTE,CANAL,DIRECCION,PLATILLOS,TOTAL\n";
    ...
    filasFiltradasGlobal.forEach(p => {
        ...
        let canalNombre = esCanalWhatsApp ? "WhatsApp" : "Mostrador";
        if (esCanalWhatsApp) cantWhatsApp++;
        else cantMostrador++;
        ...
        csvContent += `"${p.fecha || ''}",${cliente},"${canalNombre}",${direccion},${productos},${totalNum.toFixed(2)}\n`;
    });

    csvContent += "\n=== RESUMEN GENERAL ===\n";
    csvContent += `TOTAL PEDIDOS,${filasFiltradasGlobal.length}\n`;
    csvContent += `PEDIDOS WHATSAPP,${cantWhatsApp}\n`;
    csvContent += `PEDIDOS MOSTRADOR,${cantMostrador}\n`;
    csvContent += `VENTA TOTAL ACUMULADA,$${totalVentas.toFixed(2)}\n`;
    csvContent += `TICKET PROMEDIO,$${(totalVentas / filasFiltradasGlobal.length).toFixed(2)}\n`;
