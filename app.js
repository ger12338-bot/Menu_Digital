...
let promoSeleccionadaActual = null;
let cart = {};
let userCoordinates = null;
let shippingCost = 0;
let deliveryDistanceMeters = 0;
let tiendaEstaCerrada = false;
...
// ==========================================================================
// 2. LÓGICA Y CONFIGURACIÓN DEL CARRUSEL DE PROMOCIONES
// ==========================================================================
function evaluarPromosDelDia() {
    const numDia = new Date().getDay();
    const nombreDia = DIAS_TEXTO[numDia];

    promosActivasHoy = promociones.filter(p => 
    ...
    if (!promosActivasHoy || promosActivasHoy.length === 0 || !container) {
        if (container) container.style.display = "none";
        return;
    }

    renderCarruselPromociones();
    container.style.display = "block";
}

function renderCarruselPromociones() {
    ...
    promosActivasHoy.forEach((promo, idx) => {
        const precioLista = parseFloat(promo.precioLista || 0);
        const precioOferta = parseFloat(promo.precioOferta || 0);
        const ahorro = precioLista > precioOferta ? Math.round(precioLista - precioOferta) : 0;

        const bgImg = promo.imagen && promo.imagen.trim() !== "" 
            ? promo.imagen 
        ...
        const deshabilitadoAttr = tiendaEstaCerrada ? 'disabled style="opacity:0.5; cursor:not-allowed;"' : '';
        const textoBoton = tiendaEstaCerrada ? '<i class="fa-solid fa-lock"></i> Cerrado' : '<i class="fa-solid fa-cart-plus"></i> Pedir Combo';

        slide.innerHTML = `
            ${ahorro > 0 ? `<div class="badge-ahorro">¡Ahorras $${ahorro}!</div>` : ''}
            <div class="badge-tag"><i class="fa-solid fa-fire"></i> ${promo.subtitulo || 'Oferta del Día'}</div>
                ...
                <div class="slide-content">
                    ...
                    <div class="slide-action-box">
                        <div class="slide-price-container">
                            ${precioLista > 0 ? `<span class="price-old">$${precioLista.toFixed(2)}</span>` : ''}
                            <span class="price-new">$${precioOferta.toFixed(2)}</span>
                        </div>
                        <button type="button" onclick="abrirModalOpcionesPromo(${idx})" class="add-btn" ${deshabilitadoAttr}>
                            ${textoBoton}
                        </button>
                    </div>
                </div>
        ...
        dot.onclick = () => irASlideCarrusel(idx);
        dotsNav.appendChild(dot);
    });

    if (promosActivasHoy.length <= 1) {
...
function irASlideCarrusel(index) {
    if (window.innerWidth >= 768) return;
    ...
    if (index < 0) index = promosActivasHoy.length - 1;
    if (index >= promosActivasHoy.length) index = 0;
...
function moverCarrusel(step) {
    if (window.innerWidth >= 768) return;
    irASlideCarrusel(currentCarouselIndex + step);
}
...
function abrirModalOpcionesPromo(indexPromo) {
    if (tiendaEstaCerrada) return;
    
    promoSeleccionadaActual = promosActivasHoy[indexPromo];
    if (!promoSeleccionadaActual) return;

    document.getElementById("modalComboTitulo").innerText = promoSeleccionadaActual.titulo;
    const cuerpo = document.getElementById("modalComboCuerpo");
    cuerpo.innerHTML = "";

    const grupos = (promoSeleccionadaActual.configOpciones || "").split("|");
...
function confirmarAgregarComboAlCarrito() {
    ...
    let nombreItemCombo = `🔥 ${promoSeleccionadaActual.titulo} (${detalle})`;

    let itemKey = `COMBO_${Date.now()}`;
    cart[itemKey] = {
        name: nombreItemCombo,
        price: parseFloat(promoSeleccionadaActual.precioOferta),
        quantity: 1
    };
...
// ==========================================================================
// 7. ENVÍO A GOOGLE SHEETS Y WHATSAPP (CORREGIDO Y RECALCULADO)
// ==========================================================================
function sendOrder(event) {
    ...
    // RECALCULO FORZADO Y REAL EN EL MOMENTO DEL ENVIÓ
    const subtotal = calculateSubtotal();
...
function procesarEnvioWhatsApp(name, phone, address, references, subtotal, grandTotal, notes, paymentMethod, cashAmount) {
    // Asegurar el cálculo del total real sumando el costo de envío
    const totalReal = subtotal + parseFloat(shippingCost || 0);

    let pagoTexto = "";
    if (paymentMethod === "Efectivo") {
        const pagaCon = parseFloat(cashAmount) || totalReal;
        const cambio = pagaCon >= totalReal ? (pagaCon - totalReal).toFixed(2) : "0.00";
