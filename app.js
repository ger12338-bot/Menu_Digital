...
// ==========================================================================
// 5. CÁLCULO DE DISTANCIA REAL POR CALLES (API OSRM + REGLAS DE TARIFA)
// ==========================================================================
async function fetchDrivingDistanceMeters(lat1, lon1, lat2, lon2) {
    try {
        ...
        if (data.code === "Ok" && data.routes && data.routes.length > 0) {
            return data.routes[0].distance; // Metros reales por calle
        } else {
...
function calculateFallbackDistance(lat1, lon1, lat2, lon2) {
    ...
    return (R * c) * 1.35; // Respaldo: distancia aérea + 35% de margen por calles
}

function calculateShippingFee(distanceMeters) {
    // 1. Regla Vecinal: Hasta 300m es GRATIS ($0.00)
    if (distanceMeters <= 300) {
        return 0;
    }

    // 2. Tarifa base calculada: $2.00 por cada 100m ($20.00 / km)
    let costoCalculado = (distanceMeters / 100) * 2.00;

    // 3. Regla Mínima: Si el costo sale menor a $20.00, sube a $20.00
    if (costoCalculado < 20.00) {
        costoCalculado = 20.00;
    }

    // 4. Regla Redondeo: Redondear siempre hacia el entero siguiente
    return Math.ceil(costoCalculado);
}

// ==========================================================================
// 6. GPS Y ENVÍO A WHATSAPP
// ==========================================================================
function getLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
        async (position) => {
            ...
            // Consulta la distancia real por calle de forma asíncrona
...
function sendOrder(event) {
    ...
    // FORMATO DE PLATILLOS IDENTICO AL POS
