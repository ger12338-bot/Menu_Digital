...
// ==========================================================================
// IMPRESIÓN DE TICKET DE CLIENTE (MEDIDAS TÉRMICAS Y FORMATO DE COLUMNAS)
// ==========================================================================
function imprimirTicketPOS(v) {
    ...
    if (v.productos) {
        v.productos.split("|").forEach(prod => {
            let texto = prod.trim();
            if (texto !== "") {
                // Parsear formato guardado: "Cantidad x Nombre ($P.Unit c/u = $Subtotal)" o de envío
    ...
    ventanaImp.document.write(`
        ...
        <head>
            ...
            <style>
                ...
                .totals-table { width: 100%; margin-top: 5px; font-size: 11px; }
        ...
        <body>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="width: 21%; text-align: right;">Sub</th>
                    </tr>
                </thead>
                <tbody>
                    ${itemsHTML}
            ...
            <table class="totals-table">
                <tr>
                    <td class="bold" style="font-size: 12px;">TOTAL NETO:</td>
                    <td class="right bold" style="font-size: 12px;">$${parseFloat(v.total).toFixed(2)}</td>
                </tr>
                ${v.metodo_pago === 'Efectivo' ? `
                <tr>
                    <td>Paga Con:</td>
                    <td class="right">$${parseFloat(v.efectivo_recibido || v.total).toFixed(2)}</td>
                </tr>
                <tr>
                    <td>Cambio:</td>
                    <td class="right">$${parseFloat(v.cambio || 0).toFixed(2)}</td>
                </tr>
                ` : ''}
            </table>
            <div class="line"></div>
            <div class="center" style="margin-top:10px; font-size:10px;">¡Gracias por su compra!<br>¡Buen Provecho! 🔥</div>
            <script>
                window.onload = function() { window.print(); window.close(); }
            </script>
        </body>
    ...
    if (v.productos) {
        v.productos.split("|").forEach(prod => {
            ...
            if (itemTexto !== "") {
                ...
                itemsHTML += `<div style="font-size: 13px; font-weight: bold; margin: 6px 0; border-bottom: 1px dotted #ccc; padding-bottom: 4px;">• ${partesLimpias}</div>`;
            }
        });
    }
    ...
    ventanaImp.document.write(`
        ...
        <head>
            ...
            <style>
                ...
                .bold { font-weight: bold; }
                .area-tag { font-size: 14px; text-align: center; font-weight: bold; border: 2px solid #000; padding: 4px; margin-bottom: 8px; }
            </style>
        </head>
        <body>
            ...
            <div class="line"></div>
            <div class="bold" style="font-size: 12px; margin-bottom: 6px;">PLATILLOS:</div>
            ${itemsHTML}
            <div class="line"></div>
            <div><b>Notas / Instrucciones:</b><br>${v.notas || 'Ninguna'}</div>
            <div class="line"></div>
            <div class="center" style="margin-top:10px; font-size:10px;">--- ORDEN DE PRODUCCIÓN ---</div>
            <script>
                window.onload = function() { window.print(); window.close(); }
            </script>
        </body>
