...
function parsearFechaUniversal(strFecha) {
    ...
    // Si viene como fecha larga de sistema (ej. "Tue Jul 21 2026...")
...
// ==========================================================================
// 1. HISTORIAL DE VENTAS AGRUPADO Y 100% EN ESPAÑOL
// ==========================================================================
function abrirModalHistorialVentas() {
    ...
    .then(data => {
        ...
        // AGRUPAR VENTAS POR FECHA PARSEADA
        ...
        clavesOrdenadas.forEach(clave => {
            ...
            // DIBUJAR ENCABEZADO DE DÍA EN ESPAÑOL
            ...
            // ORDENAR VENTAS DEL DÍA POR HORA (MÁS RECIENTES PRIMERO)
            ...
            grupo.ventas.forEach(v => {
                ...
                card.innerHTML = `
                    <div style="display:flex; flex-direction:column; max-width:60%;">
                        ...
                        <span style="color:#888; font-size:0.75rem;">${horaCorta} hs (${v.metodo_pago})</span>
...
// ==========================================================================
// 3. ARQUEO Y CORTE DE CAJA (FILTRADO STRICTAMENTE POR TURNO ACTIVO)
// ==========================================================================
    ...
    .then(data => {
        ...
        pedidos.forEach(p => {
            if (p.estatus !== "CANCELADO") {
                ...
                // FILTRO CLAVE: Solo contar ventas registradas a partir de la apertura de este turno
