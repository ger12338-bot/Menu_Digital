...
// ==========================================================================
// GESTOR DE ESTADO Y MESAS ENCAPSULADO (FASE 2)
// Protege la memoria interna para evitar cruces de pedidos o pérdida de datos
// ==========================================================================
class GestorEstadoPOS {
    constructor() {
        this.modoActual = "mostrador"; // "mostrador" o "mesas"
        this.listaMesas = ["Mesa 1", "Mesa 2", "Mesa 3", "Mesa 4", "Mesa 5", "Mesa 6"];
        this.mesaActual = null;
        this.carritoMostrador = {};
        this.estadoMesas = this.cargarMemoriaMesas();
    }

    cargarMemoriaMesas() {
        try {
            const guardado = localStorage.getItem("POS_ESTADO_MESAS_TACOS_V2");
    ...
    guardarMemoriaMesas() {
        try {
            localStorage.setItem("POS_ESTADO_MESAS_TACOS_V2", JSON.stringify(this.estadoMesas));
        } catch(e) {
            console.error("Error al guardar estado local de mesas", e);
        }
    }

    obtenerCarritoActivo() {
        if (this.modoActual === "mesas" && this.mesaActual) {
            if (!this.estadoMesas[this.mesaActual]) {
                this.estadoMesas[this.mesaActual] = { cart: {}, notas: "" };
            }
            return this.estadoMesas[this.mesaActual].cart;
        }
        return this.carritoMostrador;
    }

    fijarCarritoActivo(nuevoCart) {
        if (this.modoActual === "mesas" && this.mesaActual) {
            if (!this.estadoMesas[this.mesaActual]) {
                this.estadoMesas[this.mesaActual] = { cart: {}, notas: "" };
            }
            this.estadoMesas[this.mesaActual].cart = nuevoCart;
        } else {
            this.carritoMostrador = nuevoCart;
        }
        this.guardarMemoriaMesas();
    }

    seleccionarMesa(nombreMesa) {
        this.mesaActual = nombreMesa;
        if (!this.estadoMesas[nombreMesa]) {
            this.estadoMesas[nombreMesa] = { cart: {}, notas: "" };
        }
    }

    limpiarConsumoActual() {
        if (this.modoActual === "mesas" && this.mesaActual) {
            delete this.estadoMesas[this.mesaActual];
            this.guardarMemoriaMesas();
        } else {
            this.carritoMostrador = {};
        }
    ...
    obtenerNotasMesa() {
        if (this.modoActual === "mesas" && this.mesaActual && this.estadoMesas[this.mesaActual]) {
            return this.estadoMesas[this.mesaActual].notas || "";
        }
        return "";
    }

    guardarNotasMesa(notas) {
        if (this.modoActual === "mesas" && this.mesaActual) {
            if (!this.estadoMesas[this.mesaActual]) {
                this.estadoMesas[this.mesaActual] = { cart: {}, notas: "" };
            }
            this.estadoMesas[this.mesaActual].notas = notas;
            this.guardarMemoriaMesas();
        }
    }
}

// Instancia única y segura del gestor
const posState = new GestorEstadoPOS();
let turnoPOS = { activo: false, idTurno: null, fondoInicial: 0, gastos: [], fechaApertura: null };
...
// ==========================================================================
// 1. HISTORIAL DE VENTAS CON DETALLE AMPLIADO Y COMANDA DE COCINA
// ==========================================================================
    ...
    .then(data => {
        ...
        clavesOrdenadas.forEach(clave => {
            ...
            grupo.ventas.forEach(v => {
                ...
                let productosTexto = (v.productos || "").replace(/\|/g, "<br>• ");
                if (productosTexto.trim() !== "") productosTexto = "• " + productosTexto;
                ...
                let notasTexto = (v.notas && v.notas !== "Venta local") ? `<br><span style="color:#ffc107;">📝 Notas: ${v.notas}</span>` : "";
                ...
                card.innerHTML = `
                    <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px dashed rgba(255,255,255,0.08); padding-bottom:8px;">
                        <div>
                            <span style="color:#fff; font-weight:800; font-size:0.95rem;">${v.cliente || v.nombre || 'General'}</span>
                            <span style="color:#888; font-size:0.75rem; display:block;">${horaCorta} hs</span>
                        </div>
                        <div style="display:flex; flex-direction:column; align-items:flex-end;">
                            <strong style="color:#ff944d; font-size:1.1rem;">$${parseFloat(v.total || 0).toFixed(2)}</strong>
                            <select class="select-metodo-historial" onchange="cambiarMetodoPagoHistorial('${fechaEscapada}', this.value, ${v.indexReal})" style="margin-top:4px;">
                    ...
                    <div style="font-size:0.82rem; color:#ccc; line-height:1.4;">
                        ${productosTexto}
                        ${notasTexto}
                    </div>

                    <div style="display:flex; justify-content:flex-end; gap:8px; border-top:1px dashed rgba(255,255,255,0.08); padding-top:8px;">
                        <button onclick="imprimirComandaCocinaGenerica(${v.indexReal})" style="background:#202430; color:#00e676; border:1px solid rgba(0,230,118,0.3); padding:6px 12px; border-radius:8px; font-size:0.78rem; font-weight:700; cursor:pointer;" title="Imprimir Comanda Cocina">
                            <i class="fa-solid fa-utensils"></i> Comanda
                        </button>
                        <button onclick="enviarVentaEspecificaWhatsApp(${v.indexReal})" style="background:#25D366; color:#fff; border:none; padding:6px 12px; border-radius:8px; font-size:0.78rem; font-weight:700; cursor:pointer;" title="Enviar por WhatsApp">
                            <i class="fa-brands fa-whatsapp"></i> WhatsApp
                        </button>
                        <button onclick="reimprimirTicketGenerico(${v.indexReal})" style="background:#222; color:#fff; border:1px solid rgba(255,255,255,0.1); padding:6px 12px; border-radius:8px; font-size:0.78rem; font-weight:700; cursor:pointer;" title="Imprimir Ticket Cliente">
...
function imprimirComandaCocinaGenerica(index) {
    const v = listaVentasGlobalesTurno[index];
    if (v) imprimirComandaCocina(v);
}
...
function abrirModalCorteCaja() {
    ...
    .then(data => {
                            ...
                            if (item !== "") {
                                ...
                                if (partes.length >= 2) {
                                    ...
                                    if (!nombreProd.includes("Envío")) {
                                        ...
                                        let prodCatalogo = products.find(pr => pr.nombre.trim().toLowerCase() === nombreProd.toLowerCase());
                                        let dueño = prodCatalogo ? (prodCatalogo.propietario || "General") : "General";
                                        
                                        let precioUnitario = prodCatalogo ? (parseFloat(prodCatalogo.precio) || 0) : 0;
                                        let subtotalProd = precioUnitario * cant;
                                        
                                        ventasPorPropietario[dueño] = (ventasPorPropietario[dueño] || 0) + subtotalProd;
                                    }
                                }
                            }
                        });
        ...
        let htmlSociosUI = "";
        let sociosKeys = Object.keys(ventasPorPropietario);
        if (sociosKeys.length === 0) {
            htmlSociosUI = "Sin ventas en este turno";
        } else {
            sociosKeys.forEach(dueño => {
                htmlSociosUI += `<div>• ${dueño}: <span style="color: var(--neon-green);">$${ventasPorPropietario[dueño].toFixed(2)}</span></div>`;
            });
        }
        const listaSociosEl = document.getElementById("listaSociosUI");
        if (listaSociosEl) listaSociosEl.innerHTML = htmlSociosUI;
...
function imprimirTicketCorte() {
    ...
    let ticketSociosHTML = "";
    // Reconstruimos las ventas por propietario del turno actual para el ticket
    let fechaAperturaObj = parsearFechaUniversal(turnoPOS.fechaApertura);
    let ventasPorPropietarioTicket = {};
    ...
    .then(data => {
        ...
        pedidos.forEach(p => {
            if (p.estatus !== "CANCELADO") {
                let fechaPedidoObj = parsearFechaUniversal(p.fecha);
                if (fechaPedidoObj >= fechaAperturaObj && p.productos) {
                    p.productos.split("|").forEach(itemRaw => {
                        let item = itemRaw.trim();
                        if (item !== "") {
                            let partes = item.split("x ");
                            if (partes.length >= 2) {
                                ...
                                if (!nombreProd.includes("Envío")) {
                                    ...
                                    let dueño = prodCatalogo ? (prodCatalogo.propietario || "General") : "General";
                                    let precioUnitario = prodCatalogo ? (parseFloat(prodCatalogo.precio) || 0) : 0;
                                    ventasPorPropietarioTicket[dueño] = (ventasPorPropietarioTicket[dueño] || 0) + (precioUnitario * cant);
                                }
                            }
                        }
                    });
        ...
        Object.keys(ventasPorPropietarioTicket).forEach(dueño => {
            ticketSociosHTML += `<div>• ${dueño}: $${ventasPorPropietarioTicket[dueño].toFixed(2)}</div>`;
        });
        if (!ticketSociosHTML) ticketSociosHTML = `<div>• General: $0.00</div>`;

        continuarImpresionCorte(totalGastos, gastosHTML, ventasEfectivo, ventasTarjeta, ventasSpei, efectivoEsperado, contado, desgloseHTML, diferencia, platilloEstrella, cantEstrella, ticketSociosHTML);
    }).catch(() => {
        ticketSociosHTML = `<div>• Desglose no disponible</div>`;
        continuarImpresionCorte(totalGastos, gastosHTML, ventasEfectivo, ventasTarjeta, ventasSpei, efectivoEsperado, contado, desgloseHTML, diferencia, platilloEstrella, cantEstrella, ticketSociosHTML);
    });
}

function continuarImpresionCorte(totalGastos, gastosHTML, ventasEfectivo, ventasTarjeta, ventasSpei, efectivoEsperado, contado, desgloseHTML, diferencia, platilloEstrella, cantEstrella, ticketSociosHTML) {
    const htmlCorte = `<html><head><title>Corte de Turno POS</title><style>
        body{font-family:monospace;width:260px;padding:10px;margin:0;color:#000;font-size:12px;font-weight:bold;}
        .center{text-align:center;}.line{border-bottom:1.5px dashed #000;margin:8px 0;}.bold{font-weight:bold;}
    </style></head><body>
        <div class="center bold" style="font-size:14px;">TACOS & PAPAS ASADAS GF</div>
        ...
        <div class="line"></div>
        <div class="bold">💰 VENTAS POR PROPIETARIO:</div>
        ${ticketSociosHTML}
        <div class="line"></div>
    ...
    imprimirIframeSilencioso(htmlCorte);
}
...
function cambiarModoPOS(modo) {
    ...
    if (modo === "mesas") {
        ...
        renderizarGridMesas();
        if (!posState.mesaActual) seleccionarMesa("Mesa 1");
    } else {
...
function renderizarGridMesas() {
    ...
    posState.listaMesas.forEach(nombreMesa => {
        const datosMesa = posState.estadoMesas[nombreMesa];
        ...
        const esSeleccionada = (posState.mesaActual === nombreMesa);
        const card = document.createElement("div");
...
function seleccionarMesa(nombreMesa) {
    posState.seleccionarMesa(nombreMesa);
    ...
    if (document.getElementById("posNotes")) {
        document.getElementById("posNotes").value = posState.obtenerNotasMesa();
    }
    renderizarGridMesas();
    actualizarUIPOS();
}
...
function guardarComandaAMesaActual() {
    if (!posState.mesaActual) return;
    const notas = document.getElementById("posNotes").value.trim();
    posState.guardarNotasMesa(notas);

    if (Object.keys(posState.obtenerCarritoActivo()).length === 0) {
        posState.limpiarConsumoActual();
    } else {
        posState.guardarMemoriaMesas();
    }
...
function imprimirPreCuentaMesa() {
    const cart = posState.obtenerCarritoActivo();
    ...
    Object.keys(cart).forEach(id => {
        ...
        subtotalCalc += sub;
        itemsHTML += `<div style="display:flex; justify-content:space-between; margin: 4px 0; font-size: 11px; font-weight: bold;"><span>${item.quantity}x ${item.name}</span><span>$${sub.toFixed(2)}</span></div>`;
    });

    const htmlPreCuenta = `<html><head><title>Pre-Cuenta ${posState.mesaActual}</title><style>
        body{font-family:monospace;width:260px;padding:10px;margin:0;font-size:12px;font-weight:bold;}.center{text-align:center;}.line{border-bottom:1.5px dashed #000;margin:8px 0;}.bold{font-weight:bold;}
    </style></head><body>
        <div class="center bold" style="font-size:14px;">TACOS & PAPAS ASADAS GF</div>
        <div class="center">*** PRE-CUENTA ***</div>
        <div class="line"></div>
        <div><b>Lugar:</b> ${posState.mesaActual}</div>
    ...
    imprimirIframeSilencioso(htmlPreCuenta);
}
...
function agregarAlCarritoPOS(productId) {
    ...
    let cart = posState.obtenerCarritoActivo();
    if (cart[productId]) {
    ...
    posState.fijarCarritoActivo(cart);
    actualizarUIPOS();
}
...
function procesarVentaPOS() {
    ...
    let etiquetaCliente = (posState.modoActual === "mesas" && posState.mesaActual) ? `Consumo en ${posState.mesaActual}` : "Venta Local Mostrador";
...
// ==========================================================================
// IMPRESIÓN TÉRMICA BLINDADA Y DE ALTA NITIDEZ (FASE 1)
// ==========================================================================
function imprimirIframeSilencioso(htmlContent) {
...
function imprimirTicketPOS(v) {
    ...
    if (v.productos) {
        v.productos.split("|").forEach(prod => {
            let texto = prod.trim();
            if (texto !== "") {
                if (texto.includes("🚚 Envío")) {
                    let partesEnvio = texto.split("($");
                    let costoEnvio = partesEnvio[1] ? parseFloat(partesEnvio[1]).toFixed(2) : "0.00";
                    itemsHTML += `<tr><td>1</td><td>Envío a Domicilio</td><td class="right">$${costoEnvio}</td><td class="right">$${costoEnvio}</td></tr>`;
                } else {
                    let partesX = texto.split("x ");
                    let cant = partesX[0] ? partesX[0].trim() : "1";
                    let resto = partesX[1] ? partesX[1] : texto;
                    let divisionPrecio = resto.split(" ($");
                    let nombreProd = divisionPrecio[0].trim();
                    let pUnit = "0.00";
                    let subTotal = "0.00";
                    
                    if (divisionPrecio[1]) {
                        let calcPartes = divisionPrecio[1].split(" c/u = $");
                        pUnit = parseFloat(calcPartes[0]).toFixed(2);
                        if (calcPartes[1]) {
                            subTotal = parseFloat(calcPartes[1].replace(")", "")).toFixed(2);
                        }
                    }
                    itemsHTML += `<tr><td>${cant}</td><td>${nombreProd}</td><td class="right">$${pUnit}</td><td class="right">$${subTotal}</td></tr>`;
                }
            }
        });
    }

    const html = `<html><head><title>Ticket Venta</title><style>
        @page{size:58mm auto;margin:0;}
        body{font-family:'Courier New',Courier,monospace;width:58mm;max-width:58mm;margin:0 auto;padding:4px;color:#000;font-size:11px;line-height:1.2;font-weight:bold;-webkit-print-color-adjust:exact;}
        .center{text-align:center;}.right{text-align:right;}.bold{font-weight:bold;}
        .line{border-bottom:1.5px dashed #000;margin:5px 0;}
        table{width:100%;border-collapse:collapse;font-size:10.5px;}
        th{text-align:left;border-bottom:1.5px solid #000;padding-bottom:2px;font-size:10.5px;font-weight:bold;}
        td{padding:2px 0;vertical-align:top;font-weight:bold;}
        .totals-table{width:100%;margin-top:4px;font-size:11.5px;}
        .totals-table td{padding:1.5px 0;}
    </style></head><body>
        <div class="center bold" style="font-size:13px;">TACOS & PAPAS ASADAS GF</div>
        <div class="center" style="font-size:10.5px;">*** TICKET DE VENTA ***</div>
        <div class="line"></div>
        <div><b>Fecha:</b> ${v.fecha}</div>
        <div><b>Lugar:</b> ${v.nombre || v.cliente || 'Mostrador'}</div>
        <div><b>Pago:</b> ${v.metodo_pago}</div>
        ${v.notas && v.notas !== "Venta local" ? `<div><b>Notas:</b> ${v.notas}</div>` : ''}
        <div class="line"></div>
        <table><thead><tr><th style="width: 12%;">Ct</th><th style="width: 46%;">Desc</th><th style="width: 21%; text-align: right;">P.U</th><th style="width: 21%; text-align: right;">Sub</th></tr></thead><tbody>${itemsHTML}</tbody></table>
        <div class="line"></div>
        <table class="totals-table">
            <tr><td class="bold" style="font-size: 12px;">TOTAL NETO:</td><td class="right bold" style="font-size: 12px;">$${parseFloat(v.total).toFixed(2)}</td></tr>
            ${v.metodo_pago === 'Efectivo' ? `<tr><td>Paga Con:</td><td class="right">$${parseFloat(v.efectivo_recibido || v.total).toFixed(2)}</td></tr><tr><td>Cambio:</td><td class="right">$${parseFloat(v.cambio || 0).toFixed(2)}</td></tr>` : ''}
        </table>
        <div class="line"></div>
        <div class="center" style="margin-top:8px; font-size:10.5px;">¡Gracias por su compra!<br>¡Buen Provecho! 🔥</div>
    </body></html>`;
    
    imprimirIframeSilencioso(html);
}

function imprimirComandaCocina(v) {
    let itemsHTML = "";
    if (v.productos) {
        v.productos.split("|").forEach(prod => {
            let itemTexto = prod.trim();
            if (itemTexto !== "") {
                let partesLimpias = itemTexto.split(" ($")[0]; 
                itemsHTML += `<div style="font-size: 13px; font-weight: bold; margin: 5px 0; border-bottom: 1.5px dotted #000; padding-bottom: 3px;">• ${partesLimpias}</div>`;
            }
        });
    }

    const html = `<html><head><title>Comanda Cocina</title><style>
        @page{size:58mm auto;margin:0;}
        body{font-family:'Courier New',Courier,monospace;width:58mm;max-width:58mm;margin:0 auto;padding:4px;color:#000;font-size:11px;font-weight:bold;-webkit-print-color-adjust:exact;}
        .center{text-align:center;}.line{border-bottom:2px solid #000;margin:6px 0;}.bold{font-weight:bold;}
        .area-tag{font-size:14px;text-align:center;font-weight:bold;border:2px solid #000;padding:3px;margin-bottom:6px;}
    </style></head><body>
        <div class="area-tag">🔥 COMANDA COCINA 🔥</div>
        <div><b>Fecha/Hora:</b> ${v.fecha}</div>
        <div><b>Destino:</b> ${v.nombre || v.cliente || 'Mostrador'}</div>
        <div class="line"></div>
        <div class="bold" style="font-size: 12px; margin-bottom: 4px;">PLATILLOS:</div>
        ${itemsHTML}
        <div class="line"></div>
        <div><b>Notas / Instrucciones:</b><br>${v.notas || 'Ninguna'}</div>
        <div class="line"></div>
        <div class="center" style="margin-top:8px; font-size:10.5px;">--- ORDEN DE PRODUCCIÓN ---</div>
    </body></html>`;
...
function agregarProductoEspecialAlCarrito(event) {
    ...
    let cart = posState.obtenerCarritoActivo();
    cart["CUSTOM_" + Date.now()] = { name: `⭐ ${nombre}`, price: precio, quantity: cantidad, propietario: "General" };
    posState.fijarCarritoActivo(cart);
    actualizarUIPOS();
